...
class particleFilter:
    ...
    def Measurement_Model(self):
        ...
        for i in range(self.numParticles):
            distance = np.array(self.model.readingMap(np.array(self.particles[i])))

            rel_distance = np.linalg.norm(distance - robot_pos)

            w_t[i] = np.exp(-(rel_distance**2) / (2*self.std))


        if sum(w_t) == 0:
            self.weights = np.ones(self.numParticles) * (1/self.numParticles)
        else:
            self.weights = w_t / sum(w_t)
    ...
    def calcPosition(self):
        ...
        norm_weights = sum(np.multiply(self.particles, self.weights))

        x = max(min(norm_weights[0], self.curMax[0]), self.curMin[0])
        y = max(min(norm_weights[1], self.curMax[1]), self.curMin[1])
